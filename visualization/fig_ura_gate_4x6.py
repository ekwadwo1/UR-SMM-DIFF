#!/usr/bin/env python3
"""
fig_ura_gate_4x6.py — Render Fig. 6 (4×6 panel) from multiple subjects
========================================================================

Takes .npy arrays produced by extract_ura_figure_data.py for 4 subjects
and generates a publication-quality 4×6 panel figure.

Rows: 4 subjects (distinct clinical scenarios)
Cols: (a) Corrupted input, (b) Variance σ²_θ, (c) Gate g, 
      (d) Seg w/o URA, (e) Seg w/ URA, (f) Ground truth

Usage:
  python fig_ura_gate_4x6.py \
      --dirs /path/to/sub1 /path/to/sub2 /path/to/sub3 /path/to/sub4 \
      --slices 64 64 64 64 \
      --contrast 3 \
      --output fig_ura_gate_4x6.pdf
"""
...
# ── Global font configuration for publication-quality rendering ──
plt.rcParams.update({
    'font.family':        'sans-serif',
    'font.sans-serif':    ['Arial', 'Helvetica', 'DejaVu Sans'],
    'font.size':          11,
...
def load_subject(data_dir, slice_idx, contrast):
    ...
    corrupted = np.load(os.path.join(data_dir, "corrupted_input.npy"))   # [4,128,128,128]
    clean     = np.load(os.path.join(data_dir, "clean_input.npy"))
    var_map   = np.load(os.path.join(data_dir, "variance_map.npy"))      # [32,32,32]
    gate_map  = np.load(os.path.join(data_dir, "gate_map.npy"))
    seg_ura   = np.load(os.path.join(data_dir, "seg_with_ura.npy"))      # [128,128,128]
    ...
    # Find best slice if requested slice has no tumor
    gt_slice = seg_gt[:, :, s]
    if gt_slice.sum() < 50:
        # Auto-find slice with most tumor voxels
...
def main():
    parser = argparse.ArgumentParser("Render 4×6 URA Gate Figure")
    parser.add_argument("--dirs", nargs="+", required=True,
                        help="4 directories with extracted .npy arrays")
    ...
    parser.add_argument("--output", type=str, default="fig_ura_gate_4x6.pdf")
    parser.add_argument("--dpi", type=int, default=600)
    parser.add_argument("--row-labels", nargs="+", default=None,
                        help="Optional row labels (e.g., 'Large WT' 'Small ET' ...)")
    args = parser.parse_args()

    n_rows = len(args.dirs)
    assert n_rows >= 2, "Need at least 2 subjects"

    if args.slices is None:
        args.slices = [64] * n_rows
    assert len(args.slices) == n_rows, "Number of slices must match number of dirs"
    ...
    fig_h = 3.6 * n_rows + 1.4  # adaptive height (more room for titles/legend)
    fig = plt.figure(figsize=(19.0, fig_h), dpi=args.dpi)
    gs = gridspec.GridSpec(n_rows, 6, wspace=0.04, hspace=0.10,
                           left=0.06, right=0.995, top=1.0 - 0.55/fig_h,
                           bottom=0.55/fig_h)
    ...
    for row, subj in enumerate(subjects):
        ...
        ax.axis('off')
        if row == 0:
            ax.set_title(col_titles[0], fontsize=16, fontweight="bold", pad=10)

        # Row label (left side)
        if args.row_labels and row < len(args.row_labels):
            label = args.row_labels[row]
        else:
            # Use short subject ID
            sid = subj["sid"]
        ...
        ax.axis('off')
        if row == 0:
            ax.set_title(col_titles[1], fontsize=16, fontweight="bold", pad=10)
        ...
        ax.axis('off')
        if row == 0:
            ax.set_title(col_titles[2], fontsize=16, fontweight="bold", pad=10)
        ...
        ax.axis('off')
        if row == 0:
            ax.set_title(col_titles[3], fontsize=16, fontweight="bold", pad=10)
        ...
        ax.axis('off')
        if row == 0:
            ax.set_title(col_titles[4], fontsize=16, fontweight="bold", pad=10)
        ...
        ax.axis('off')
        if row == 0:
            ax.set_title(col_titles[5], fontsize=16, fontweight="bold", pad=10)
    ...
    fig.legend(handles=legend_patches, loc='lower center', ncol=3, fontsize=14,
               frameon=True, fancybox=False, edgecolor='0.65',
               bbox_to_anchor=(0.5, 0.002), handlelength=1.5,
               handletextpad=0.5, columnspacing=2.0)

    # ── Colorbars (below row 0, columns 1 and 2) ──
    cb_y = 1.0 - 0.50/fig_h - 3.6/fig_h + 0.015
    # Variance colorbar
    cax1 = fig.add_axes([0.21, cb_y, 0.11, 0.022])
    cb1 = fig.colorbar(im_var, cax=cax1, orientation='horizontal')
    cb1.set_ticks([])
    cb1.outline.set_linewidth(1.0)
    cb1.ax.text(0.0, -2.2, 'Low', fontsize=12, fontweight='bold',
                ha='left', va='top', transform=cb1.ax.transAxes)
    cb1.ax.text(1.0, -2.2, 'High', fontsize=12, fontweight='bold',
                ha='right', va='top', transform=cb1.ax.transAxes)

    # Gate colorbar
    sm = plt.cm.ScalarMappable(cmap=gate_cmap, norm=plt.Normalize(0, 1))
    cax2 = fig.add_axes([0.375, cb_y, 0.11, 0.022])
    cb2 = fig.colorbar(sm, cax=cax2, orientation='horizontal')
    cb2.set_ticks([0, 1])
    cb2.set_ticklabels(['0 (suppress)', '1 (pass)'], fontsize=11,
                        fontweight='bold')
