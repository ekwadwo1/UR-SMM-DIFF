...
class PhysicsConsistentDiffusion(nn.Module):
    ...
    def ddim_sample(
        ...
        """
        Full modified DDIM reverse sampling with physics-aware init.

        CRITICAL FIX: Cannot start from pure noise because the physics-
        consistent forward process at t=T yields z_T = z₀^phys (not
        Gaussian noise). At high t, λ_t → 1 and ᾱ_t → 0, making the
        effective-frame decomposition numerically degenerate.

        Solution: cap DDIM start at t_start = t_start_frac * T where
        (1−λ) and √ᾱ are well-conditioned. Initialise z_t from the
        forward marginal q(z_{t_start} | z_obs, z_obs) using z_obs as
        a proxy for both z₀ and z₀^phys:
        ...
        # z_{t_s} = (1−λ)·√ᾱ·z_obs + λ·z_obs + (1−λ)·√(1−ᾱ)·ε
        #         = ((1−λ)·√ᾱ + λ)·z_obs + (1−λ)·√(1−ᾱ)·ε
        t_s = torch.tensor([t_start], device=device)
        lam_s     = self._extract(self.lambdas, t_s)           # [1,1,1,1,1]
        sqrt_ab_s = self._extract(self.sqrt_ab, t_s)
        sqrt_1m_s = self._extract(self.sqrt_one_m_ab, t_s)

        signal_coeff = (1.0 - lam_s) * sqrt_ab_s + lam_s      # z_obs weight
        noise_coeff  = (1.0 - lam_s) * sqrt_1m_s              # noise weight

        noise = torch.randn(shape, device=device)
        z_t = signal_coeff * z_obs + noise_coeff * noise       # [B,c,h,w,d]
